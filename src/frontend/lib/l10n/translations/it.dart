import '../app_text_key.dart';

const Map<AppTextKey, String> itTranslations = <AppTextKey, String>{
  AppTextKey.appTitle: 'CarNine',
  AppTextKey.navHome: 'Home',
  AppTextKey.navMaps: 'Mappe',
  AppTextKey.navMedia: 'Media',
  AppTextKey.navClimate: 'Clima',
  AppTextKey.navControls: 'Comandi',
  AppTextKey.navSettings: 'Opzioni',
  AppTextKey.navHomeSemantic: 'Dashboard home',
  AppTextKey.navMapsSemantic: 'Mappe',
  AppTextKey.navMediaSemantic: 'Lettore multimediale',
  AppTextKey.navClimateSemantic: 'Comandi climatizzazione',
  AppTextKey.navControlsSemantic: 'Comandi ausiliari e illuminazione',
  AppTextKey.navSettingsSemantic: 'Opzioni',
  AppTextKey.emergency: 'EMERGENZA',
  AppTextKey.showFrontendLogs: 'Mostra log frontend',
  AppTextKey.frontendLogs: 'Log frontend',
  AppTextKey.noLogEntriesYet: 'Nessuna voce di log',
  AppTextKey.close: 'Chiudi',
  AppTextKey.dashboardContentFor: 'Contenuto dashboard per {section}',
  AppTextKey.grpcStatus: 'Stato gRPC: {status}',
  AppTextKey.statusNotConnected: 'Non connesso',
  AppTextKey.statusConnecting: 'Connessione...',
  AppTextKey.statusConnected: 'Connesso - {count} dati ricevuti',
  AppTextKey.statusError: 'Errore',
  AppTextKey.grpcConnectionErrorTitle: 'Errore di connessione',
  AppTextKey.grpcConnectionErrorMessage:
      'Connessione al backend non riuscita. Riprova oppure controlla '
      'i log di diagnostica.',
  AppTextKey.testGrpc: 'Test gRPC',
  AppTextKey.connecting: 'Connessione',
  AppTextKey.canDataLine: 'Sensore: {sensor}, valore: {value}, ora: {time}',
  AppTextKey.mediaQueueTitle: 'In coda',
  AppTextKey.mediaCollectionsTitle: 'Raccolte',
  AppTextKey.mediaPreviousSemantic: 'Traccia precedente',
  AppTextKey.mediaNextSemantic: 'Traccia successiva',
  AppTextKey.mediaPlaySemantic: 'Riproduci',
  AppTextKey.mediaPauseSemantic: 'Pausa',
  AppTextKey.mediaRewind30Semantic: 'Indietro di 30 secondi',
  AppTextKey.mediaForward30Semantic: 'Avanti di 30 secondi',
  AppTextKey.mediaQueueExpandSemantic: 'Mostra coda',
  AppTextKey.mediaQueueCollapseSemantic: 'Nascondi coda',
  AppTextKey.mediaShuffleSemantic: 'Riproduzione casuale',
  AppTextKey.mediaRepeatSemantic: 'Ripeti',
  AppTextKey.mediaRepeatOffSemantic: 'Ripetizione: disattivata',
  AppTextKey.mediaRepeatQueueSemantic: 'Ripetizione: playlist',
  AppTextKey.mediaRepeatTrackSemantic: 'Ripetizione: brano corrente',
  AppTextKey.mediaVolumeSemantic: 'Volume {percent} percento',
  AppTextKey.mediaMuteSemantic: 'Disattiva audio',
  AppTextKey.mediaUnmuteSemantic: 'Riattiva audio',
  AppTextKey.mediaAudioErrorBanner: 'Si è verificato un errore audio',
  AppTextKey.mediaAudioDeviceChangedBanner:
      'Dispositivo di uscita audio cambiato',
  AppTextKey.mediaCreateAction: 'Crea',
  AppTextKey.mediaCreateSemantic: 'Crea nuova raccolta',
  AppTextKey.mediaCollectionsSemantic: 'Apri raccolte',
  AppTextKey.mediaBackToPlayerSemantic: 'Torna al lettore',
  AppTextKey.mediaUnknownTitle: 'Titolo sconosciuto',
  AppTextKey.mediaUnknownArtist: 'Artista sconosciuto',
  AppTextKey.mediaNothingPlaying: 'Nessuna riproduzione',
  AppTextKey.mediaQueuePositionLine: 'TRACCIA {index} DI {count}',
  AppTextKey.mediaSingleTrackLine: 'TRACCIA SINGOLA',
  AppTextKey.mediaStopSemantic: 'Ferma la riproduzione',
  AppTextKey.mediaFeatureUnavailableHint: 'non ancora disponibile',
  AppTextKey.mediaQueueEmpty: 'La coda è vuota',
  AppTextKey.mediaNoAdjacentTrack: 'Nessun\'altra traccia in coda',
  AppTextKey.mediaLoading: 'Caricamento...',
  AppTextKey.mediaRetry: 'Riprova',
  AppTextKey.mediaOfflineTitle: 'Nessuna connessione',
  AppTextKey.mediaOfflineDescription:
      'La connessione al backend è stata interrotta. Riconnessione '
      'automatica in corso.',
  AppTextKey.mediaBackendErrorTitle: 'Errore',
  AppTextKey.mediaBackendErrorDescription: 'Questa richiesta non è riuscita.',
  AppTextKey.mediaCommandFailed: 'Comando non riuscito',
  AppTextKey.mediaDismiss: 'Chiudi',
  AppTextKey.mediaLibraryTitle: 'Libreria',
  AppTextKey.mediaSearchHint: 'Cerca titolo o artista',
  AppTextKey.mediaSearchSemantic: 'Cerca nella libreria',
  AppTextKey.mediaSearchClearSemantic: 'Cancella ricerca',
  AppTextKey.mediaLibraryEmpty:
      'La libreria è vuota. Avvia una scansione per importare i '
      'contenuti multimediali.',
  AppTextKey.mediaSearchNoResults: 'Nessun risultato per questa ricerca',
  AppTextKey.mediaRescanAction: 'Riscansiona',
  AppTextKey.mediaRescanSemantic: 'Riscansiona la libreria',
  AppTextKey.mediaUsbImportBanner:
      'Chiavetta USB "{label}" trovata - importare {count} brano/i?',
  AppTextKey.mediaUsbImportAction: 'Importa',
  AppTextKey.mediaUsbImportSemantic: 'Importa {count} brano/i da {label}',
  AppTextKey.mediaScanRunning: 'Scansione in corso...',
  AppTextKey.mediaScanProgressLine:
      '{processed} elaborati, {imported} importati',
  AppTextKey.mediaScanFailed: 'Scansione non riuscita',
  AppTextKey.mediaUnavailableBadge: 'NON DISPONIBILE',
  AppTextKey.mediaUnavailableSemantic: '{title}, non disponibile',
  AppTextKey.mediaPlayTrackSemantic: 'Riproduci {title}',
  AppTextKey.mediaPlaylistsTitle: 'Playlist',
  AppTextKey.mediaPlaylistsEmpty: 'Nessuna playlist ancora',
  AppTextKey.mediaPlaylistOpenSemantic: 'Apri playlist {name}',
  AppTextKey.mediaPlaylistPlaySemantic: 'Riproduci playlist {name}',
  AppTextKey.mediaPlaylistPlayAction: 'Riproduci playlist',
  AppTextKey.mediaPlaylistDetailEmpty: 'Questa playlist non ha ancora tracce',
  AppTextKey.mediaPlaylistEntryUnknown: 'Traccia non più nella libreria',
  AppTextKey.mediaPlaylistNameHint: 'Nome della playlist',
  AppTextKey.mediaPlaylistNameRequired: 'Inserisci un nome',
  AppTextKey.mediaPlaylistExistsError:
      'Esiste già una playlist con questo nome',
  AppTextKey.mediaPlaylistCreateAction: 'Crea playlist',
  AppTextKey.mediaPlaylistAddEntriesTitle: 'Aggiungi tracce',
  AppTextKey.mediaPlaylistAddEntryAction: 'Aggiungi tracce',
  AppTextKey.mediaPlaylistTrackAlreadyAdded: 'Il brano è già nella playlist',
  AppTextKey.mediaPlaylistAddEntrySemantic: 'Aggiungi {title} a {playlist}',
  AppTextKey.mediaBackToCollectionsSemantic: 'Torna alle raccolte',
  AppTextKey.settingsBackToOptions: 'Torna alle opzioni',
  AppTextKey.settingsLanguageTitle: 'Lingua',
  AppTextKey.settingsLanguageSubtitle: 'Scegli la lingua di visualizzazione',
  AppTextKey.settingsLanguageSemantic: 'Apri le impostazioni lingua',
  AppTextKey.settingsDiagnosticsTitle: 'Sistema',
  AppTextKey.settingsDiagnosticsSubtitle: 'Log, riavvio e aggiornamenti',
  AppTextKey.settingsDiagnosticsSemantic: 'Apri le impostazioni di sistema',
  AppTextKey.settingsAppearanceTitle: 'Aspetto',
  AppTextKey.settingsAppearanceSubtitle: 'Colori e tipografia',
  AppTextKey.settingsAppearanceSemantic: 'Apri impostazioni aspetto',
  AppTextKey.settingsMapTitle: 'Impostazioni mappa',
  AppTextKey.settingsMapSubtitle: 'Vista mappa e comportamento dei percorsi',
  AppTextKey.settingsMapSemantic: 'Apri impostazioni mappa',
  AppTextKey.settingsCurrentLanguage: 'Lingua attiva',
  AppTextKey.settingsDiagnosticsRecentLogs: 'Log frontend recenti',
  AppTextKey.settingsDiagnosticsOpenLogs: 'Apri visualizzatore log',
  AppTextKey.settingsDiagnosticsExit: 'Esci',
  AppTextKey.settingsDiagnosticsRestart: 'Riavvia',
  AppTextKey.settingsDiagnosticsCheckUpdates: 'Cerca aggiornamenti',
  AppTextKey.settingsDiagnosticsUpdatesUnavailableTitle:
      'Non ancora disponibile',
  AppTextKey.settingsDiagnosticsUpdatesUnavailableMessage:
      'La ricerca di aggiornamenti non è ancora implementata.',
  AppTextKey.settingsExitPasswordTitle: 'Password richiesta',
  AppTextKey.settingsExitPasswordHint: 'Password',
  AppTextKey.settingsExitPasswordIncorrect: 'Password errata',
  AppTextKey.settingsExitPasswordConfirmAction: 'Conferma',
  AppTextKey.settingsExitPasswordCancelAction: 'Annulla',
  AppTextKey.settingsAppearanceComingSoonTitle: 'Aspetto preparato',
  AppTextKey.settingsAppearanceComingSoonDescription:
      'Colori e tipografia saranno configurati in questa area.',
  AppTextKey.settingsMapComingSoonTitle: 'Impostazioni mappa preparate',
  AppTextKey.settingsMapComingSoonDescription:
      'Vista mappa e opzioni di percorso saranno configurate qui.',
  AppTextKey.settingsMockDescription:
      'Area temporanea delle opzioni globali del frontend.',
  AppTextKey.languageGerman: 'tedesco',
  AppTextKey.languageEnglish: 'inglese',
  AppTextKey.languageFrench: 'francese',
  AppTextKey.languageSpanish: 'spagnolo',
  AppTextKey.languageItalian: 'italiano',
  AppTextKey.languageChinese: 'cinese',
  AppTextKey.languageJapanese: 'giapponese',
  AppTextKey.languageDutch: 'olandese',
  AppTextKey.languagePolish: 'polacco',
  AppTextKey.languageHungarian: 'ungherese',
  AppTextKey.languageTurkish: 'turco',
  AppTextKey.languagePortuguese: 'portoghese',
  AppTextKey.languageCzech: 'ceco',
  AppTextKey.languageSwedish: 'svedese',
  AppTextKey.languageDanish: 'danese',
  AppTextKey.keyboardDoneAction: 'Fine',
  AppTextKey.keyboardLettersLayerLabel: 'ABC',
  AppTextKey.keyboardLettersLayerSemantic: 'Passa alle lettere',
  AppTextKey.keyboardNumbersLayerLabel: '123',
  AppTextKey.keyboardNumbersLayerSemantic: 'Passa a numeri e simboli',
  AppTextKey.keyboardShiftSemantic: 'Maiuscolo',
  AppTextKey.keyboardBackspaceSemantic: 'Elimina carattere',
  AppTextKey.keyboardSpaceSemantic: 'Spazio',
  AppTextKey.keyboardDiacriticOptionsSemantic: 'Altre opzioni per {letter}',
  AppTextKey.mapsSearchPlaceholder: 'Inserisci destinazione',
  AppTextKey.mapsNextTurnLabel: 'Prossima svolta',
  AppTextKey.mapsArrivalLabel: 'Arrivo',
  AppTextKey.mapsArrivalTomorrowLabel: 'Arrivo domani',
  AppTextKey.mapsDurationLabel: 'Durata',
  AppTextKey.mapsDistanceLabel: 'Distanza',
  AppTextKey.mapsCancelRouteAction: 'ANNULLA',
  AppTextKey.mapsZoomInSemantic: 'Aumenta zoom',
  AppTextKey.mapsZoomOutSemantic: 'Riduci zoom',
  AppTextKey.mapsRecenterSemantic: 'Centra sulla posizione',
  AppTextKey.mapsSearchSemantic: 'Cerca destinazione',
  AppTextKey.mapsSearchClearSemantic: 'Cancella ricerca',
  AppTextKey.mapsNavigationModeSemantic: 'Modalità navigazione: direzione in alto',
  AppTextKey.mapsNorthUpSemantic: 'Orienta il nord in alto',
  AppTextKey.mapsRouteCalculating: 'Calcolo del percorso …',
  AppTextKey.mapsRoutingOffline: 'Navigazione non disponibile',
  AppTextKey.mapsNoRouteFound: 'Nessun percorso trovato',
  AppTextKey.mapsNoPosition: 'Nessuna posizione GPS',
  AppTextKey.mapsRouteError: 'Impossibile calcolare il percorso',
  AppTextKey.mapsSearchNoResults: 'Nessun risultato',
  AppTextKey.mapsTypePlace: 'Luogo',
  AppTextKey.mapsTypeRegion: 'Regione',
  AppTextKey.mapsTypePoi: 'POI',
  AppTextKey.mapsTypePeak: 'Vetta',
  AppTextKey.mapsTypeWater: 'Acqua',
  AppTextKey.mapsTypeStreet: 'Strada',
  AppTextKey.mapsNoMapData: 'Nessun dato mappa installato',
  AppTextKey.mapsMapUnavailable: 'Impossibile visualizzare la mappa',
};
