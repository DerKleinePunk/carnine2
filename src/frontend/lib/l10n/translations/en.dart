import '../app_text_key.dart';

const Map<AppTextKey, String> enTranslations = <AppTextKey, String>{
  AppTextKey.appTitle: 'CarNine',
  AppTextKey.navHome: 'Home',
  AppTextKey.navMaps: 'Maps',
  AppTextKey.navMedia: 'Media',
  AppTextKey.navClimate: 'Climate',
  AppTextKey.navControls: 'Controls',
  AppTextKey.navSettings: 'Settings',
  AppTextKey.navHomeSemantic: 'Home dashboard',
  AppTextKey.navMapsSemantic: 'Maps',
  AppTextKey.navMediaSemantic: 'Media player',
  AppTextKey.navClimateSemantic: 'Climate controls',
  AppTextKey.navControlsSemantic: 'Auxiliary controls',
  AppTextKey.navSettingsSemantic: 'Settings',
  AppTextKey.emergency: 'EMERGENCY',
  AppTextKey.showFrontendLogs: 'Show frontend logs',
  AppTextKey.frontendLogs: 'Frontend Logs',
  AppTextKey.noLogEntriesYet: 'No log entries yet',
  AppTextKey.close: 'Close',
  AppTextKey.dashboardContentFor: 'Dashboard content for {section}',
  AppTextKey.grpcStatus: 'gRPC Status: {status}',
  AppTextKey.statusNotConnected: 'Not connected',
  AppTextKey.statusConnecting: 'Connecting...',
  AppTextKey.statusConnected: 'Connected - received {count} data points',
  AppTextKey.statusError: 'Error',
  AppTextKey.grpcConnectionErrorTitle: 'Connection Error',
  AppTextKey.grpcConnectionErrorMessage:
      'Failed to connect to the backend. Please try again or check the '
      'diagnostics logs.',
  AppTextKey.testGrpc: 'Test gRPC',
  AppTextKey.connecting: 'Connecting',
  AppTextKey.canDataLine: 'Sensor: {sensor}, Value: {value}, Time: {time}',
  AppTextKey.mediaQueueTitle: 'Next in Queue',
  AppTextKey.mediaCollectionsTitle: 'Collections',
  AppTextKey.mediaPreviousSemantic: 'Previous track',
  AppTextKey.mediaNextSemantic: 'Next track',
  AppTextKey.mediaPlaySemantic: 'Play',
  AppTextKey.mediaPauseSemantic: 'Pause',
  AppTextKey.mediaRewind30Semantic: 'Rewind 30 seconds',
  AppTextKey.mediaForward30Semantic: 'Forward 30 seconds',
  AppTextKey.mediaQueueExpandSemantic: 'Show queue',
  AppTextKey.mediaQueueCollapseSemantic: 'Hide queue',
  AppTextKey.mediaShuffleSemantic: 'Shuffle',
  AppTextKey.mediaRepeatSemantic: 'Repeat',
  AppTextKey.mediaRepeatOffSemantic: 'Repeat: off',
  AppTextKey.mediaRepeatQueueSemantic: 'Repeat: playlist',
  AppTextKey.mediaRepeatTrackSemantic: 'Repeat: current track',
  AppTextKey.mediaVolumeSemantic: 'Volume {percent} percent',
  AppTextKey.mediaMuteSemantic: 'Mute',
  AppTextKey.mediaUnmuteSemantic: 'Unmute',
  AppTextKey.mediaAudioErrorBanner: 'An audio error occurred',
  AppTextKey.mediaAudioDeviceChangedBanner: 'Audio output device changed',
  AppTextKey.mediaCreateAction: 'Create',
  AppTextKey.mediaCreateSemantic: 'Create new collection',
  AppTextKey.mediaCollectionsSemantic: 'Open collections',
  AppTextKey.mediaBackToPlayerSemantic: 'Back to player',
  AppTextKey.mediaUnknownTitle: 'Unknown title',
  AppTextKey.mediaUnknownArtist: 'Unknown artist',
  AppTextKey.mediaNothingPlaying: 'Nothing playing',
  AppTextKey.mediaQueuePositionLine: 'TRACK {index} OF {count}',
  AppTextKey.mediaSingleTrackLine: 'SINGLE TRACK',
  AppTextKey.mediaStopSemantic: 'Stop playback',
  AppTextKey.mediaFeatureUnavailableHint: 'not yet available',
  AppTextKey.mediaQueueEmpty: 'Queue is empty',
  AppTextKey.mediaNoAdjacentTrack: 'No adjacent track in the queue',
  AppTextKey.mediaLoading: 'Loading...',
  AppTextKey.mediaRetry: 'Retry',
  AppTextKey.mediaOfflineTitle: 'No connection',
  AppTextKey.mediaOfflineDescription:
      'The connection to the backend was lost. Reconnecting automatically.',
  AppTextKey.mediaBackendErrorTitle: 'Error',
  AppTextKey.mediaBackendErrorDescription: 'This request failed.',
  AppTextKey.mediaCommandFailed: 'Command failed',
  AppTextKey.mediaDismiss: 'Dismiss',
  AppTextKey.mediaLibraryTitle: 'Library',
  AppTextKey.mediaSearchHint: 'Search title or artist',
  AppTextKey.mediaSearchSemantic: 'Search library',
  AppTextKey.mediaSearchClearSemantic: 'Clear search',
  AppTextKey.mediaLibraryEmpty:
      'The library is empty. Start a rescan to import media.',
  AppTextKey.mediaSearchNoResults: 'No results for this search',
  AppTextKey.mediaRescanAction: 'Rescan',
  AppTextKey.mediaRescanSemantic: 'Rescan library',
  AppTextKey.mediaUsbImportBanner:
      'USB drive "{label}" found - import {count} track(s)?',
  AppTextKey.mediaUsbImportAction: 'Import',
  AppTextKey.mediaUsbImportSemantic: 'Import {count} track(s) from {label}',
  AppTextKey.mediaScanRunning: 'Scan running...',
  AppTextKey.mediaScanProgressLine:
      '{processed} processed, {imported} imported',
  AppTextKey.mediaScanFailed: 'Scan failed',
  AppTextKey.mediaUnavailableBadge: 'UNAVAILABLE',
  AppTextKey.mediaUnavailableSemantic: '{title}, unavailable',
  AppTextKey.mediaPlayTrackSemantic: 'Play {title}',
  AppTextKey.mediaPlaylistsTitle: 'Playlists',
  AppTextKey.mediaPlaylistsEmpty: 'No playlists yet',
  AppTextKey.mediaPlaylistOpenSemantic: 'Open playlist {name}',
  AppTextKey.mediaPlaylistPlaySemantic: 'Play playlist {name}',
  AppTextKey.mediaPlaylistPlayAction: 'Play playlist',
  AppTextKey.mediaPlaylistDetailEmpty: 'This playlist has no tracks yet',
  AppTextKey.mediaPlaylistEntryUnknown: 'Track no longer in the library',
  AppTextKey.mediaPlaylistNameHint: 'Playlist name',
  AppTextKey.mediaPlaylistNameRequired: 'Please enter a name',
  AppTextKey.mediaPlaylistExistsError:
      'A playlist with this name already exists',
  AppTextKey.mediaPlaylistCreateAction: 'Create playlist',
  AppTextKey.mediaPlaylistAddEntriesTitle: 'Add tracks',
  AppTextKey.mediaPlaylistAddEntryAction: 'Add tracks',
  AppTextKey.mediaPlaylistTrackAlreadyAdded: 'Track is already in the playlist',
  AppTextKey.mediaPlaylistAddEntrySemantic: 'Add {title} to {playlist}',
  AppTextKey.mediaBackToCollectionsSemantic: 'Back to collections',
  AppTextKey.settingsBackToOptions: 'Back to options',
  AppTextKey.settingsLanguageTitle: 'Language',
  AppTextKey.settingsLanguageSubtitle: 'Choose display language',
  AppTextKey.settingsLanguageSemantic: 'Open language settings',
  AppTextKey.settingsDiagnosticsTitle: 'System',
  AppTextKey.settingsDiagnosticsSubtitle: 'Logs, restart and updates',
  AppTextKey.settingsDiagnosticsSemantic: 'Open system settings',
  AppTextKey.settingsAppearanceTitle: 'Appearance',
  AppTextKey.settingsAppearanceSubtitle: 'Colors and typography',
  AppTextKey.settingsAppearanceSemantic: 'Open appearance settings',
  AppTextKey.settingsMapTitle: 'Map settings',
  AppTextKey.settingsMapSubtitle: 'Map view and route behavior',
  AppTextKey.settingsMapSemantic: 'Open map settings',
  AppTextKey.settingsCurrentLanguage: 'Active language',
  AppTextKey.settingsDiagnosticsRecentLogs: 'Recent frontend logs',
  AppTextKey.settingsDiagnosticsOpenLogs: 'Open log viewer',
  AppTextKey.settingsDiagnosticsExit: 'Exit',
  AppTextKey.settingsDiagnosticsRestart: 'Restart',
  AppTextKey.settingsDiagnosticsCheckUpdates: 'Check for updates',
  AppTextKey.settingsDiagnosticsUpdatesUnavailableTitle: 'Not available yet',
  AppTextKey.settingsDiagnosticsUpdatesUnavailableMessage:
      'Checking for updates is not implemented yet.',
  AppTextKey.settingsExitPasswordTitle: 'Password required',
  AppTextKey.settingsExitPasswordHint: 'Password',
  AppTextKey.settingsExitPasswordIncorrect: 'Incorrect password',
  AppTextKey.settingsExitPasswordConfirmAction: 'Confirm',
  AppTextKey.settingsExitPasswordCancelAction: 'Cancel',
  AppTextKey.settingsAppearanceComingSoonTitle: 'Appearance prepared',
  AppTextKey.settingsAppearanceComingSoonDescription:
      'Colors and typography will be configured in this area.',
  AppTextKey.settingsMapComingSoonTitle: 'Map settings prepared',
  AppTextKey.settingsMapComingSoonDescription:
      'Map view and routing options will be configured in this area.',
  AppTextKey.settingsMockDescription:
      'Temporary settings area for global frontend options.',
  AppTextKey.languageGerman: 'German',
  AppTextKey.languageEnglish: 'English',
  AppTextKey.languageFrench: 'French',
  AppTextKey.languageSpanish: 'Spanish',
  AppTextKey.languageItalian: 'Italian',
  AppTextKey.languageChinese: 'Chinese',
  AppTextKey.languageJapanese: 'Japanese',
  AppTextKey.languageDutch: 'Dutch',
  AppTextKey.languagePolish: 'Polish',
  AppTextKey.languageHungarian: 'Hungarian',
  AppTextKey.languageTurkish: 'Turkish',
  AppTextKey.languagePortuguese: 'Portuguese',
  AppTextKey.languageCzech: 'Czech',
  AppTextKey.languageSwedish: 'Swedish',
  AppTextKey.languageDanish: 'Danish',
  AppTextKey.keyboardDoneAction: 'Done',
  AppTextKey.keyboardLettersLayerLabel: 'ABC',
  AppTextKey.keyboardLettersLayerSemantic: 'Switch to letters',
  AppTextKey.keyboardNumbersLayerLabel: '123',
  AppTextKey.keyboardNumbersLayerSemantic: 'Switch to numbers and symbols',
  AppTextKey.keyboardShiftSemantic: 'Shift',
  AppTextKey.keyboardBackspaceSemantic: 'Delete character',
  AppTextKey.keyboardSpaceSemantic: 'Space',
  AppTextKey.keyboardDiacriticOptionsSemantic: 'More options for {letter}',
  AppTextKey.mapsSearchPlaceholder: 'Enter destination',
  AppTextKey.mapsNextTurnLabel: 'Next Turn',
  AppTextKey.mapsArrivalLabel: 'Arrival',
  AppTextKey.mapsDurationLabel: 'Duration',
  AppTextKey.mapsDistanceLabel: 'Distance',
  AppTextKey.mapsCancelRouteAction: 'CANCEL',
  AppTextKey.mapsZoomInSemantic: 'Zoom in',
  AppTextKey.mapsZoomOutSemantic: 'Zoom out',
  AppTextKey.mapsRecenterSemantic: 'Recenter on location',
  AppTextKey.mapsSearchSemantic: 'Search destination',
  AppTextKey.mapsSearchClearSemantic: 'Clear search',
  AppTextKey.mapsNavigationModeSemantic: 'Navigation mode: heading up',
  AppTextKey.mapsNorthUpSemantic: 'Align north up',
  AppTextKey.mapsRouteCalculating: 'Calculating route …',
  AppTextKey.mapsRoutingOffline: 'Navigation unavailable',
  AppTextKey.mapsNoRouteFound: 'No route found',
  AppTextKey.mapsNoPosition: 'No GPS position',
  AppTextKey.mapsRouteError: 'Route could not be calculated',
  AppTextKey.mapsSearchNoResults: 'No results',
  AppTextKey.mapsNoMapData: 'No map data installed',
  AppTextKey.mapsMapUnavailable: 'The map cannot be displayed',
};
